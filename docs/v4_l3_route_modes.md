...
- **분기 수와 퇴행의 상관은 +0.025** — 사실상 0이다. "분기가 적어 모드가 굶는다"는
  가설은 반증된다.
- **가장 큰 단일 덩어리는 지도가 경로를 못 주는 75개**(3.75%)다: 기준선 0.710 → l3b 1.154,
  퇴행 **+0.445**, 전체 퇴행의 **14.2%**. 이 시나리오들은 폴백이 직진 하나뿐이라
  사실상 `minADE1` 로 채점됐다. → 부채꼴 폴백(`v4_l3c`)이 고치는 부분.
...
> minADE6 이 0.710 으로 **전체 평균보다 훨씬 쉬운** 시나리오다. 모집단 절제표(1모드 = 3.458)를
> 여기에 곱하면 4배쯤 과대평가된다. 그 버킷을 기준선 수준까지 **완벽히** 고쳐도 전체는
> 1.414 − 0.0375 × (1.154 − 0.710) ≈ **1.397 이 상한**이다.
>
