...
**왜 가속도만 유독 심한가** — 미분할 때마다 노이즈가 `1/dt` 배로 커지기 때문입니다.
위치 측정오차를 σ라 하면:

| 파생량 | 노이즈 std | dt=0.1 일 때 |
| --- | --- | --- |
| 속력 `Δp/dt` | √2·σ/dt | 14.1·σ |
| 가속도 `ΔΔp/dt²` | √6·σ/dt² | **244.9·σ** |

가속도는 속력보다 `√3/dt = 17.3배` 더 노이즈에 민감합니다. 실제 데이터로 분해해 보면:

| 항목 | 값 |
| --- | --- |
| 위치차분 가속도 std (신호+노이즈) | 4.44 m/s² |
| AV2 velocity 필드 기반 가속도 std (신호로 간주) | 1.91 m/s² |
| 노이즈 성분 | 4.01 m/s² = **전체 분산의 81%** |
| 역산한 위치 측정오차 σ | **1.6 cm** |

**1.6cm 의 위치 오차가 가속도에서는 4 m/s² 로 증폭됩니다.**

**lag-1 자기상관 0.353 을 읽는 법** — 연속된 두 값이 얼마나 닮았는지의 척도입니다. 기준점:

| 신호 | lag-1 자기상관 |
| --- | --- |
| 완전히 매끄러운 가속도 | +1.00 |
| AV2 velocity 필드 기반 | +0.79 |
| **위치차분 가속도 (원본)** | **+0.34** |
| 백색잡음을 두 번 차분한 것 (순수 노이즈) | −0.67 |

순수 노이즈는 0이 아니라 **−0.67** 입니다(두 번 차분하면 이웃끼리 반대 부호가 됨).
그러니 0.34는 "완전한 백색잡음"은 아니고 **신호와 노이즈가 섞였는데 노이즈가 분산의 81%를
차지하는 상태**입니다.
...
메커니즘은 이렇습니다. **스무딩한 a 를 적분하면 원래 속력이 아니라 "스무딩된 속력"이 나옵니다.**
그 속력 잔차가 시간에 곱해져 위치 오차가 됩니다.

| 창 | `\|MA(v) − v\|` RMS | × T/2 (=예측) | 실측 ADE |
| --- | --- | --- | --- |
| 3 | 0.171 m/s | 0.43 m | 0.58 m |
| 5 | 0.241 m/s | 0.60 m | 1.03 m |
| 9 | 0.383 m/s | 0.96 m | 1.87 m |

예측과 실측이 같은 크기로 맞습니다. (2배 안쪽 차이는 잔차가 구간 안에서 완전히 상쇄되지 않기 때문)

> 참고로 스무딩 오차가 **누적되지는 않습니다.** 이동평균의 누적합은 경계를 빼면 원본과 같아서,
> 위치 오차가 `0.5·Δa·T²`(창 5면 13m)처럼 커지지 않고 1m 수준에 머뭅니다.

### ① 안이 이 함정을 피한 방법

**v 는 원본을 쓰고, a 만 스무딩합니다.**

- `v` 채널: 스무딩 안 함 → 복원 무손실 유지 (원본과 차이 0)
- `a` 채널: 창 5 스무딩 → lag-1 자기상관 0.34 → **0.91**, 신호가 됨

**② 안은 이 분리가 불가능합니다.** 채널이 하나뿐이라 그 채널이 속도까지 책임져야 하는데,
a 를 스무딩하면 누적합이 v 를 못 만듭니다. 결국 노이즈를 그대로 안고 학습하게 되고,
이것도 ② 가 진 이유 중 하나로 보입니다.
