...
- **출발 상태 (s0, d0) 캐시 결함 (09-17, 미수정):** 모드의 약 0.18% 는 첫 예측점이 차량에서 멀리(최대 36 m) 떨어져 시작한다. 모델 출력이 아니라 캐시의 `route_sd0` 결함이다(`dataset_lane` → `lane_frame.to_frame` 전진 대응). 궤적 그림은 원점 → 첫 점을 점선으로 그리고, 좌표 점검 기준은 0.5 m 로 둔다.
- **정지 차량 heading 뒤집힘 (09-17, 미수정):** `align_ref` 가 위치 잡음으로 정지 차량을 180° 뒤집는다(`docs/v4_heading_prep_viz.md`). focal 10 Hz 입력 0.72% 에도 들어 있다.
- **정답과 비교할 때는 같은 처리를 건다:** 정답 가속도는 속도 필드를 평활한 값이다. 모델 원출력과 그대로 비교하면 불공정하다(09-17 검토 지적).
- **원인 단정 금지:** 두 조건이 함께 바뀐 비교(예: 에폭 수 + 학습률 스케줄)는 "뒤반(학습률 감소 + 추가 학습)"처럼 묶어서 부르고, 대조판이 없다고 적는다.
